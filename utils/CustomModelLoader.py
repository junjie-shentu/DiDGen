...
from utils.CustomAttnProcessor import CustomDiffusionAttnProcessor2_0, CustomDiffusionAttnProcessor
...
# in this version of Diffusers, 'from diffusers.utils.constants import DIFFUSERS_CACHE' is not available, so we need to define it here
...
class CustomModelLoader(UNet2DConditionLoadersMixin):
    ...
    def load_attn_procs_train(self, pretrained_model_name_or_path_or_dict: Union[str, Dict[str, torch.Tensor]], train_q, train_k, train_v, train_out, **kwargs):
        r"""
        ...
        if is_lora:
            ...
            for key, value_dict in custom_diffusion_grouped_dict.items():
                if len(value_dict) == 0:
                    attn_processors[key] = CustomDiffusionAttnProcessor2_0(
                    ...
                    attn_processors[key] = CustomDiffusionAttnProcessor2_0(    # here,the train_k, train_v, train_q, train_out are temporarily manually set, we can set them to be parameters in the future.
                        train_k=train_k,
                        train_v=train_v,
                        train_q=train_q,
                        train_out=train_out,
    ...
    def load_attn_procs_inference(self, pretrained_model_name_or_path_or_dict: Union[str, Dict[str, torch.Tensor]], train_q, train_k, train_v, train_out, controller,**kwargs):
        r"""
        This is from the original "load_attn_procs" method in the UNet2DConditionLoadersMixin class. We delete the lora part.
        Load pretrained attention processor layers into [`UNet2DConditionModel`]. Attention processor layers have to be
        defined in
