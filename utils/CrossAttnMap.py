...
def show_all_cross_attention_maps(tokenizer, prompts, attention_store: AttentionStore, res: int, from_where: List[str], num_prompts: int):
    ...
    for j in range(num_prompts):
        ...
        for i in range(len(tokens)):
            ...
            #-------------------
            max_attention  = image.max() # this is to help us understand the scale of the attention
            #-------------------
            image = 255 * image / image.max()
            image = image.unsqueeze(-1).expand(*image.shape, 3)
            image = image.numpy().astype(np.uint8)
            image = np.array(Image.fromarray(image).resize((256, 256)))
            image = text_under_image(image, decoder(int(tokens[i])))#text_under_image(image, (decoder(int(tokens[i]))+f'max_attn:{max_attention}'))#text_under_image(image, decoder(int(tokens[i])))
            images.append(image)
        cross_attention = view_images(np.stack(images, axis=0))
