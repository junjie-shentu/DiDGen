...
"""
This version has endured some changes to the original code. 
Although the file name remains the same, it should be noted that the code has been modified to fit the new structure of the project.
"""
...
class AttentionStore(AttentionControl):
        ...
        if len(self.attention_store["down_cross"]) == 0:
            ...
            for key in self.attention_store:
                for i in range(len(self.attention_store[key])):
                    ...
                    self.attention_store[key][i] = self.attention_store[key][i] + self.step_store[key][i].detach()
    ...
    def back_up_attention_store(self):
        # return the whole attention store (sum of all steps) and cur_step
        return self.attention_store, self.cur_step
    
    def restore_attention_store(self, attention_store, cur_step):
        # restore the back-up attention store and cur_step
...
def show_cross_attention_specific_token(token_pos, prompts, attention_store: AttentionStore, res: int, from_where: List[str], select: int = 0):
    ...
    image = image.unsqueeze(-1).expand(*image.shape, 3)
    image = image.numpy().astype(np.uint8)
    image = np.array(Image.fromarray(image).resize((512, 512)))#(256, 256)
    images.append(image)
    cross_attention = view_images(np.stack(images, axis=0))
    return cross_attention
